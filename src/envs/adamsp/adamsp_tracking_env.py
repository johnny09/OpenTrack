...
from src.envs.adamsp import base_env as adamsp_base
from src.envs.adamsp import adamsp_tracking_constants as consts
...
def get_feet_contact(self, data: mjx.Data) -> jax.Array:
    ...
    left_contacts = jp.array(
        [
            geoms_colliding(data, geom_id, self.geom_id_floor)
...
class AdamSPTrackingEnv(adamsp_base.AdamSPEnv):
    @property
    ...
    def _post_init(self) -> None:
        ...
        waist_indices = []
        waist_joint_names = ["waist_roll", "waist_pitch", "waist_yaw"]
        ...
        self._feet_all_site_id = jp.array(
            [self._mj_model.site(name).id for name in consts.FEET_ALL_SITES]
        )
        self._hands_site_id = jp.array(
        ...
        self.shoulder_ids = jp.array(
            [self.mj_model.body(n).id for n in consts.SHOULDER_LINKs]
        )
        self.valid_body_ids = jp.concatenate(
            (self.lower_body_ids, self.upper_body_ids)
        )  # link of id 0 is world!

        self.dof_pos_lower_limit = jp.array(
            [item[0] for item in consts.RESTRICTED_JOINT_RANGE]
        )
        self.dof_pos_upper_limit = jp.array(
            [item[1] for item in consts.RESTRICTED_JOINT_RANGE]
        )
    ...
    def reset(
        ...
        carry = self.th.reset_state_with_trajectory(
            trajectory_data, TrajCarry(rng, self.th.init_state())
        )
        ...
        traj_data = self.th.get_current_traj_data_with_trajectory(
            trajectory_data, carry
        )
        ...
        if self._config.history_len > 0:
            ...
            init_history_action = jp.concatenate(
                [init_history, info["last_motor_targets"]], axis=0
            )
            obs["history_state"] = jp.stack(
    ...
    def step(
        ...
        traj_data = self.th.get_current_traj_data_with_trajectory(
            trajectory_data, state.info["traj_info"]
        )
        lower_motor_targets = (
            traj_data.qpos[7:][self.action_joint_ids]
        ...
        rewards = self._get_reward(
            data, traj_data, action, motor_targets, torque, state.info
        )
        rewards = {
            k: v * rewards[k] for k, v in self._config.reward_config.scales.items()
        }
        ...
        state.info["traj_info"] = self.th.update_state_with_trajectory(
            trajectory_data, state.info["traj_info"]
        )
        traj_data = self.th.get_current_traj_data_with_trajectory(
            trajectory_data, state.info["traj_info"]
        ...
        if self._config.history_len > 0:
            obs["history_state"] = jp.concatenate(
                [
                    state.obs["history_state"].reshape(self._config.history_len, -1)[
                        1:
                    ],
                    jp.concatenate(
                        [
        ...
        state = jax.lax.cond(
            ...
            partial(self._reset_and_update_state, trajectory_data=trajectory_data),
            lambda x: x,
            state,
        )
        return state
    ...
    def _reset_and_update_state(
        ...
        reset_rng, current_state.info["rng"] = jax.random.split(
            current_state.info["rng"]
        )
        reset_state = self.reset(reset_rng, trajectory_data)
        current_state = current_state.replace(
            data=reset_state.data, obs=reset_state.obs
        )
    ...
    def load_trajectory(self, traj: Trajectory = None, warn: bool = True) -> None:
        ...
        return TrajectoryHandler(
            model=self._mj_model, warn=warn, traj=traj, control_dt=self.dt, **th_params
        )
        ...
        for dataset_name, traj_names in dataset_dict.items():
            ...
            for idx, t_name in enumerate(tqdm(traj_names)):
                ...
                if not traj.data.is_complete:
                    ...
                    traj.save(
                        traj_path
                    )  # save trajectory before recalculating velocity
    ...
    def extend_motion(self, traj: Trajectory) -> Trajectory:
        ...
        traj_data, traj_info = interpolate_trajectories(
            traj.data, traj.info, 1.0 / self.dt
        )
        ...
        callback = ExtendTrajData(
            self, model=self._mj_model, n_samples=traj_data.n_samples
        )
    ...
    def play_trajectory(
        ...
        for i in range(n_episodes):
            if n_steps_per_episode is None:
                nspe = (
                    self.th.len_trajectory(traj_info.traj_state.traj_no)
                    - traj_info.traj_state.subtraj_step_no
                )
            else:
    ...
    def set_sim_state_from_traj_data(
        self, data: mujoco.MjData, traj_data: TrajectoryData, carry: TrajCarry
    ) -> MjData:
        """
        ...
        """
        robot_free_jnt_qpos_id_xy = np.array(mj_jntname2qposid("root", self._mj_model))[
            :2
        ]
    ...
    def _get_termination(
        self, data: mjx.Data, traj_data: TrajectoryData, info: dict[str, Any]
    ) -> jax.Array:
        # fall_termination = jp.abs(data.qpos[2] - traj_data.qpos[2]) > self._config.termination_config.root_height_threshold
        fall_termination = (
            jp.abs(
                data.xpos[self.shoulder_ids[0], 2]
                - traj_data.xpos[self.shoulder_ids[0], 2]
            )
        ...
        dif_rigid_body_pos_local = gmth.calculate_dif_rigid_body_pos_local(
            data, traj_data
        )
        norm_dif_rigid_body_pos_local = jp.linalg.norm(
            dif_rigid_body_pos_local[self.valid_body_ids, :], axis=-1
        )
        rigid_body_position_termination = jp.any(
            norm_dif_rigid_body_pos_local
            > self._config.termination_config.rigid_body_dif_threshold
        )
    ...
    def _get_obs(
        ...
        ref_gvec_pelvis = traj_data.site_xmat[self._pelvis_imu_site_id].reshape(
            3, 3
        ).T @ jp.array([0, 0, -1])
        ...
        dif_rigid_body_rot_local = gmth.calculate_dif_rigid_body_rot_local(
            data, traj_data
        ).flatten()
        dif_rigid_body_linvel_local = gmth.calculate_dif_rigid_body_linvel_local(
            data, traj_data
        ).flatten()
        dif_rigid_body_angvel_local = gmth.calculate_dif_rigid_body_angvel_local(
            data, traj_data
        ).flatten()
        ...
        dif_root_linvel = (
            traj_root_rot_mat.T @ traj_data.qvel[:3] - root_rot_mat.T @ data.qvel[:3]
        )
        dif_root_angvel = traj_data.qvel[3:6] - data.qvel[3:6]

        ref_feet_height = traj_data.site_xpos[self._feet_all_site_id, 2]
        dif_feet_height = (
            traj_data.site_xpos[self._feet_all_site_id, 2]
            - data.site_xpos[self._feet_all_site_id, 2]
        )
        ...
        privileged_state_dict = {
            ...
            * self._config.obs_scales_config.joint_vel,
            ...
            "dif_rigid_body_linvel_local": dif_rigid_body_linvel_local
            * self._config.obs_scales_config.joint_vel,
            "dif_rigid_body_angvel_local": dif_rigid_body_angvel_local
            * self._config.obs_scales_config.joint_vel,
            "dif_root_linvel": dif_root_linvel
            * self._config.obs_scales_config.joint_vel,
            "dif_root_angvel": dif_root_angvel
            * self._config.obs_scales_config.joint_vel,
            # aux / world model ref
            "ref_gyro_pelvis": ref_gyro_pelvis
            * self._config.obs_scales_config.joint_vel,
        ...
        privileged_state = jp.hstack(
            [privileged_state_dict[k] for k in self._config.privileged_obs_keys]
        )
    ...
    def _get_reward(
        ...
        if self._config.recalculate_velocity:
            dif_rigid_body_linvel_local = (
                gmth.calculate_dif_rigid_body_linvel_local_differential(
                    data, traj_data, info["last_rigid_body_pos"]
                )
            )
            dif_rigid_body_angvel_local = (
                gmth.calculate_dif_rigid_body_angvel_local_differential(
                    data, traj_data, info["last_rigid_body_ori"]
                )
            )
            dif_joint_vel = traj_data.qvel[6:] - (
                (data.qpos[7:] - info["last_dof_pos"]) / self.dt
            )
            dif_root_linvel = gmth.calculate_dif_root_linvel_local_differential(
                data, traj_data, info["last_root_pos"]
            )
            dif_root_angvel = gmth.calculate_dif_root_angvel_local_differential(
                data, traj_data, info["last_root_ori"]
            )
        else:
            traj_root_rot_mat = math.quat_to_mat(traj_data.qpos[3:7])
            root_rot_mat = math.quat_to_mat(data.qpos[3:7])
            ...
            dif_root_linvel = (
                traj_root_rot_mat.T @ traj_data.qvel[:3]
                - root_rot_mat.T @ data.qvel[:3]
            )
            dif_root_angvel = traj_data.qvel[3:6] - data.qvel[3:6]
        ...
        reward_dict = {
            # tracking reward
            "rigid_body_pos_tracking_upper": self._reward_rigid_body_pos_tracking_upper(
                dif_rigid_body_pos_local
            ),
            "rigid_body_pos_tracking_lower": self._reward_rigid_body_pos_tracking_lower(
                dif_rigid_body_pos_local
            ),
            "feet_pos_tracking": self._reward_feet_pos_tracking(
                dif_rigid_body_pos_local
            ),
            "rigid_body_rot_tracking": self._reward_rigid_body_rot_tracking(
                dif_rigid_body_rot_local
            ),
            "rigid_body_linvel_tracking": self._reward_rigid_body_linvel_tracking(
                dif_rigid_body_linvel_local
            ),
            "rigid_body_angvel_tracking": self._reward_rigid_body_angvel_tracking(
                dif_rigid_body_angvel_local
            ),
            ...
            "penalty_action_rate": self._reward_penalty_action_rate(
                motor_targets, info["last_motor_targets"]
            ),
            ...
            "feet_linvel_deviation": self._reward_feet_linvel_deviation(
                data, global_ref_feet_vel
            ),
            "feet_angvel_deviation": self._reward_feet_angvel_deviation(
                data, global_ref_feet_vel
            ),
            ...
            "smoothness_joint": self._reward_smoothness_joint(
                data, info["last_joint_vel"]
            ),
        }

        reward_dict = jax.tree_util.tree_map(
            lambda x: jp.where(jp.isnan(x), 0.0, x), reward_dict
        )

        return reward_dict

    def _reward_rigid_body_pos_tracking_upper(
        self, dif_rigid_body_pos: jax.Array
    ) -> jax.Array:
        upper_body_diff = dif_rigid_body_pos[self.upper_body_ids, :]
        diff_body_pos_dist_upper = jp.sum(jp.abs(upper_body_diff), axis=(-2, -1))
        r_body_pos_upper = jp.exp(
            -diff_body_pos_dist_upper
            / self._config.reward_config.auxiliary.upper_body_sigma
        )

        return r_body_pos_upper

    def _reward_rigid_body_pos_tracking_lower(
        self, dif_rigid_body_pos: jax.Array
    ) -> jax.Array:
        lower_body_diff = dif_rigid_body_pos[self.lower_body_ids, :]
        diff_body_pos_dist_lower = jp.sum(jp.abs(lower_body_diff), axis=(-2, -1))
        r_body_pos_lower = jp.exp(
            -diff_body_pos_dist_lower
            / self._config.reward_config.auxiliary.lower_body_sigma
        )
    ...
    def _reward_feet_pos_tracking(self, dif_rigid_body_pos: jax.Array) -> jax.Array:
        ...
        rew = jp.exp(
            -feet_pos_dist / self._config.reward_config.auxiliary.feet_pos_sigma
        )
        return rew

    def _reward_rigid_body_rot_tracking(
        self, dif_rigid_body_rot: jax.Array
    ) -> jax.Array:
        diff_body_rot_dist = 2 * jp.arccos(
            dif_rigid_body_rot[self.valid_body_ids, 0]
        ).mean(axis=-1)
        rew = jp.exp(
            -diff_body_rot_dist / self._config.reward_config.auxiliary.body_rot_sigma
        )
        return rew

    def _reward_rigid_body_linvel_tracking(
        self, dif_rigid_body_linvel: jax.Array
    ) -> jax.Array:
        diff_body_linvel_dist = (
            dif_rigid_body_linvel[self.valid_body_ids, :] ** 2
        ).mean(axis=(-1, -2))

        rew = jp.exp(
            -diff_body_linvel_dist
            / self._config.reward_config.auxiliary.body_linvel_sigma
        )
        return rew

    def _reward_rigid_body_angvel_tracking(
        self, dif_rigid_body_angvel: jax.Array
    ) -> jax.Array:
        diff_body_angvel_dist = (
            dif_rigid_body_angvel[self.valid_body_ids, :] ** 2
        ).mean(axis=(-1, -2))

        rew = jp.exp(
            -diff_body_angvel_dist
            / self._config.reward_config.auxiliary.body_angvel_sigma
        )
        return rew
    ...
    def _reward_joint_pos_tracking(self, dif_joint_pos: jax.Array) -> jax.Array:
        ...
        rew = jp.exp(
            -diff_joint_pos_dist / self._config.reward_config.auxiliary.joint_pos_sigma
        )
        return rew
    ...
    def _reward_joint_vel_tracking(self, dif_joint_vel: jax.Array) -> jax.Array:
        ...
        rew = jp.exp(
            -diff_joint_vel_dist / self._config.reward_config.auxiliary.joint_vel_sigma
        )
        return rew
    ...
    def _reward_root_linvel_tracking(self, dif_root_linvel: jax.Array) -> jax.Array:
        ...
        rew = jp.exp(
            -diff_root_linvel_dist
            / self._config.reward_config.auxiliary.root_linvel_sigma
        )
        return rew
    ...
    def _reward_root_angvel_tracking(self, dif_root_angvel: jax.Array) -> jax.Array:
        ...
        rew = jp.exp(
            -diff_root_angvel_dist
            / self._config.reward_config.auxiliary.root_angvel_sigma
        )
        return rew
    ...
    def _reward_roll_pitch_tracking(self, dif_rp: jax.Array) -> jax.Array:
        ...
        rew = jp.exp(
            -diff_rp_dist / self._config.reward_config.auxiliary.roll_pitch_sigma
        )
        return rew
    ...
    def _reward_collision(self, data: mjx.Data) -> jax.Array:
        ...
        collided_values = jax.vmap(partial(geoms_colliding, data))(
            pair_geom1, pair_geom2
        )

        return jp.sum(collided_values, axis=-1)

    def _reward_feet_linvel_deviation(
        self, data: mjx.Data, global_ref_feet_vel: jax.Array
    ) -> jax.Array:
        current_global_feet_vel = jp.linalg.norm(data.cvel[self.feet_ids, 3:], axis=-1)
        slipping_conditions = (
            jp.abs(global_ref_feet_vel)
            < self._config.reward_config.auxiliary.global_feet_vel_threshold
        )
        left_foot_penalty = jax.lax.cond(
            slipping_conditions[0],
            lambda _: jp.square(current_global_feet_vel[0]),
            lambda _: 0.0,
            None,
        )
        right_foot_penalty = jax.lax.cond(
            slipping_conditions[1],
            lambda _: jp.square(current_global_feet_vel[1]),
            lambda _: 0.0,
            None,
        )
    ...
    def _reward_feet_angvel_deviation(
        self, data: mjx.Data, global_ref_feet_vel: jax.Array
    ) -> jax.Array:
        current_global_feet_vel = jp.linalg.norm(data.cvel[self.feet_ids, :3], axis=-1)
        slipping_conditions = (
    ...
    def _reward_root_height_tracking(self, dif_root_height: jax.Array) -> jax.Array:
        ...
        rew = jp.exp(
            -diff_root_height_dist
            / self._config.reward_config.auxiliary.root_height_sigma
        )
        return rew
    ...
    def _reward_feet_height_tracking(self, dif_feet_height: jax.Array) -> jax.Array:
        ...
        rew = jp.exp(
            -diff_feet_height_dist
            / self._config.reward_config.auxiliary.feet_height_sigma
        )
        return rew
